...
export class Puissance4 {
  ...
  initJeu() {
    ...
    options.forEach((option) => {
      this.selectJ1.value = couleurs.find(
        (couleur) => couleur.label === "Rouge"
      ).value;
    ...
    options2.forEach((option) => {
      this.selectJ2.value = couleurs.find(
        (couleur) => couleur.label === "Noir"
      ).value;
    ...
    this.joueurs = [
      new Joueur(this.selectJ1.value),
      new Joueur(this.selectJ2.value),
    ];
    jouer.onclick = () => {
      if (this.selectJ1.value === this.selectJ2.value) {
        alert(
          "Pour la clarté du jeu, vous ne pouvez pas avoir la même couleur."
        );
      } else {
        this.initPlateau();
      }
  ...
  initPlateau() {
    ...
    this.affichageJoueurActuel.innerHTML =
      "Au tour du joueur " + (this.joueurActuel + 1);
    ...
    if (this.jeuFini) {
      ...
      if (this.checkVictoire()) {
        setTimeout(() => {
          ...
          alert(
            "Le joueur " + (this.joueurActuel + 1) + " a gagné la partie !"
          );
          this.jeuFini = true;
          this.ajoutScore();
          this.reset();
      ...
      for (let col = 0; col < this.grille.cols; col++) {
        ...
        for (const direction of directions) {
          ...
          if (nbr >= 4) {
            ...
            let pion2 = {
              y: currentRow - direction.col,
              x: currentCol - direction.row,
            };
            let pion3 = {
              y: currentRow - direction.col * 2,
              x: currentCol - direction.row * 2,
            };
            let pion4 = {
              y: currentRow - direction.col * 3,
              x: currentCol - direction.row * 3,
            };
